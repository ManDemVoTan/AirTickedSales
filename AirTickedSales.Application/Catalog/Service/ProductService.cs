...
using AirTickedSales.Uilities.Exceptions;
...
namespace AirTickedSales.Application.Catalog.Service
{
    public class ProductService : IProductService
    {
        ...
        {
            var product = await _context.Products.FindAsync(productId);
            if (product == null) throw new AirTicketException($"Can not find a product {productId}");
        ...
        public async Task<ProductImageViewModel> GetImageById(int imageId)
        {
            ...
            if (image == null) throw new AirTicketException($"Cannot find an image width id {imageId}");
        ...
        public async Task<int> RemoveImage( int imageId)
        {
            ...
            if (productImage == null) throw new AirTicketException($"cannot find an image width id {imageId}");
        ...
        public async Task<int> Update(ProductUpdateRequest request)
        {
            ...
            if (product == null || productTranslations == null) throw new AirTicketException($"Cannot find a product with id: {request.Id}");
            ...
            if (request.ThumbnailImage != null)
            {
                var thumbailImage = await _context.ProductImages.FirstOrDefaultAsync(x => x.IsDefault == true && x.ProductId == request.Id);
        ...
        public async Task<int> UpdateImage(int imageId, ProductImageUpdateRequest request)
        {
            ...
            if (productImage == null)
                throw new AirTicketException($"cannot find an image widt id {imageId} ");
            if (request.ImageFile != null)
            {
                productImage.ImagePath = await this.SaveFile(request.ImageFile);
                productImage.FileSize = request.ImageFile.Length;
